...
#[derive(Debug, Clone)]
pub enum SnarfTcpTrackerError {
    FutureQueueOverflow,
    InvalidWindInput,
    InvalidRiseInput,
    PacketOutsideWindow,
}
use SnarfTcpTrackerError::*;

type Result<T> = std::result::Result<T, SnarfTcpTrackerError>;
...
/// (Write buffer, Retransmission copy, Closing)
pub type TcpTrackerRiseResult<'a> = (
...
#[derive(Clone, Debug)]
pub struct TcpTrackerWindInput {
    flags: u8,
    seq: u32,
    fresh_data_len: usize,
    rise_cnt: u64,
    closing: bool,
}

impl TcpTrackerWindInput {
    fn new(flags: u8, seq: u32, fresh_data_len: usize, rise_cnt: u64) -> Self {
        Self {
            flags,
            seq,
            fresh_data_len,
    ...
    fn new_closing(flags: u8, seq: u32, fresh_data_len: usize, rise_cnt: u64) -> Self {
        Self {
            flags,
            seq
            fresh_data_len,
    ...
    fn to_rise_and_rewind(self) -> (Option<TcpTrackerRiseInput>, TcpTrackerRewindInput) {
        let rewind_input = TcpTrackerRewindInput {
            wind: self.clone(),
        };

        if self.closing {
            return (None, rewind_input);
        }

        (
            Some(TcpTrackerRiseInput {
                flags: self.flags,
                seq: self.seq,
                fresh_data_len: self.fresh_data_len,
                rise_cnt: self.rise_cnt,
            }),
            rewind_input
        )
    }
...
#[derive(Clone, Debug)]
pub struct TcpTrackerRewindInput {
    wind: TcpTrackerWindInput,
}

pub enum TcpTrackerPlungeResult {
    NewConnection,
    Drop,
    Future,
    Closing(TcpTrackerWindInput),
    OldData,
    Wind(TcpTrackerWindInput),
}
...
// A struct that should be only created by TcpPeerTracker::plunge
#[derive(Clone, Debug)]
pub struct TcpTrackerRiseInput {
    flags: u8,
    seq: u32,
    fresh_data_len: usize,
    rise_cnt: u64,
    ...
    RF: TransportPacketParent,
{
    ...
    /// Plunge takes the packet and decides if it's ready for rising
    pub fn plunge(
        &self,
        flags: u8,
        seq: u32,
        data: &mut [u8],
    ) -> Result<TcpTrackerPlungeResult> {
        /*
        ...
        */
        ...
        let curr_seq = self.wind_seq as i64;
        ...
        if (flags & SYN_MASK) != 0 && (!self.first_seq || self.fin || self.closed) {
            if self.fin || self.closed {
                return Ok(TcpTrackerPlungeResult::NewConnection);
            }
            return Ok(TcpTrackerPlungeResult::Wind(TcpTrackerWindInput::new(flags, seq, data_len, self.wind_cnt)));
        } else if self.closed {
            return Ok(TcpTrackerPlungeResult::Drop);
        } else if (flags & RST_MASK) != 0 {
            return Ok(TcpTrackerPlungeResult::Closing(TcpTrackerWindInput::new_closing(flags, seq, data_len, self.wind_cnt)));
        } else if !self.first_seq {
            debug!("snarf: FIRST");
            // The connection has not encountered SYN yet so we consider all packets to be future
            return Ok(TcpTrackerPlungeResult::Future);
        }
        ...
        let new_data_len = (seq + data_len - curr_seq) % SEQ_LIM;
        // If old data or outside the window
        if !(1..=buffer_len).contains(&new_data_len) {
            ...
            // seq_end is in the window, but not seq
            if right_offset + data_len > buffer_len {
                return Err(PacketOutsideWindow);
            }

            self.buffer
                .write_from_buffer_to_slice(data, seq as u32);

            debug!("snarf: OLD");
            return Ok(TcpTrackerPlungeResult::OldData);
        }
        ...
        if old_data_len > 0 {
            self.buffer
                .write_from_buffer_to_slice(&mut data[..old_data_len as usize], seq as u32);
        }

        let fresh_seq = (seq as u32).wrapping_add(old_data_len as u32);
        let fresh_data_len = (data_len - old_data_len) as usize;

        return Ok(TcpTrackerPlungeResult::Wind(TcpTrackerWindInput::new(flags, fresh_seq, fresh_data_len, self.wind_cnt)));
    }

    pub fn wind<'a>(
        &mut self,
        wind_input: TcpTrackerWindInput,
    ) -> Result<(Option<TcpTrackerRiseInput>, TcpTrackerRewindInput)> {
        if wind_input.rise_cnt != self.wind_cnt {
            return Err(InvalidWindInput);
        }

        self.wind_cnt += 1;
        self.wind_seq = self.wind_seq.wrapping_add(wind_input.fresh_data_len as u32);
        if wind_input.closing {
            self.closed = true;
        }

        Ok(wind_input.to_rise_and_rewind())
    }

    pub fn rewind<'a>(
        &'a mut self,
        input: TcpTrackerRewindInput,
    )  {
        let input = input.wind;
        self.wind_cnt = input.rise_cnt;
        self.wind_seq = input.seq;
        if input.closing {
            self.closed = false;
        }
    }
    ...
    pub fn rise<'a>(
        &'a mut self,
        rise_input: TcpTrackerRiseInput,
    ) -> Result<TcpTrackerRiseResult<'a>> {
        /*
         * Assumptions:
         * 1. plunge was called before with the same parameters
         * 2. the data len is smaller than the buffer len
         * 3. the packet is not a future
         * 4. the data is fresh in its entirety
         * 5. either this is a SYN packet or self.closed is false
         * 
         * The seq still needs to be checked because the caller might 
         *
         * */
        if rise_input.rise_cnt != self.rise_cnt {
            return Err(InvalidRiseInput);
        }
        self.rise_cnt += 1;

        let flags = rise_input.flags;
        let data_len = rise_input.fresh_data_len;
        let buffer_len = self.buffer.len() as i64;
        ...
        let mut increase_data_len = data_len;
        if (flags & FIN_MASK) != 0 {
            debug!("snarf: FIN");
            self.fin = true;
            increase_data_len += 1;
        ...
        let (mut write_space, remaining_space) = self
            .buffer
            .update_and_return_split_ref(increase_data_len);
    ...
    pub fn add_future_payload(&mut self, mut rf: RF) -> std::result::Result<(), (RF, SnarfTcpTrackerError)>
    where
        RF: TransportPacketParent,
    {
        let (_, tcp_payload) = rf.split();
